...
- 机核化条目由脚本判定，审稿人只复核不重查。机核脚本盘点：`check_quotes.py` **已有**；`check_basics.py`（合并 YAML 头/行宽/图片路径/重复 n-gram/段长/违禁词断言）**M1 新建**；精读机核自 `scripts/bak/check-jingdu-sea.py` 改造，按篇配置快照路径；
